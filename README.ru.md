...
*Обновляетесь с 1.29 или раньше или раньше?* Те версии шифровали загрузки на диске вашим
`MINIO_ENC_KEY`. Оставьте этот ключ в `.env`: новым он ничего не шифрует, но файлам,
которые уже лежат в бакете, он нужен, чтобы их можно было прочитать.
...
Сохраните и `.env`. Если в нём остался `MINIO_ENC_KEY` от старой установки — он важнее
всего: файлы, загруженные тогда, без него не прочитать.
