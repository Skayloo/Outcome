...
- **Всё в реальном времени** — сообщения по WebSocket: ответы, реакции, закрепы,
  ...
  перетаскиванием.
- **Голосовые сообщения** — запись в канал или личку; сервер нормализует в m4a, чтобы
  играло на всех платформах, с перематываемой waveform (веб + мобилка).
- **Голосовые и видео-каналы** — групповой голос, камеры, демонстрация экрана
  (LiveKit SFU). Медиа шифруется в канале связи (DTLS-SRTP) между участником и SFU.
- **Гостевые ссылки в голос** — позвать человека в голосовой канал по ссылке, без
  регистрации.
- **Личные сообщения, друзья, блокировки** — переписка один на один с тем же набором
  возможностей, что и в каналах.
...
- **Настоящее нативное мобильное приложение** для iOS и Android — не обёртка над
  браузером: голосовые каналы, звонки и переписка, push-to-talk — весь функционал в
...
- **Защита входа** — 2FA (TOTP/почта), хэширование паролей Argon2id, управление
  сессиями с кнопкой «отозвать все», сброс пароля по коду на почту, опциональный
...
| [MinIO](https://hub.docker.com/r/minio/minio) | Хранилище загруженных файлов |
...
- Голос и видео шифруются в канале связи (DTLS-SRTP) между участником и SFU, который
  расшифровывает поток, чтобы его маршрутизировать. Гостевые ссылки (вход в голосовой
  канал без аккаунта) работают из коробки.

### Что эта редакция шифрует, а что нет

Это **открытая self-hosted редакция**, и на диске она не шифрует ничего. Шифруется
*канал связи* (HTTPS/WSS через Caddy, DTLS-SRTP для медиа) — и это всё: текст сообщений
лежит в вашем Postgres, а загруженные файлы в вашем MinIO ровно в том виде, в каком
пришли. Сервер ваш и диски ваши — но знать об этом стоит до того, как вы пообещаете
пользователям обратное.

**Сквозное шифрование**, при котором сам сервер не может прочитать личную переписку и
услышать звонок, в эту редакцию не входит. Оно есть в коммерческой редакции — напишите
нам, если оно вам нужно.

**Обновляетесь с 1.29 или раньше, и у вас задан `MINIO_ENC_KEY`?** Те версии шифровали
загрузки на диске. В этой расшифровки нет вовсе, поэтому переведите бакет **до** обновления —
после него эти файлы станут шифротекстом, который нечем открыть:

```bash
docker run --rm --network <сеть вашего compose> \
  -e MINIO_ENDPOINT=minio:9000 -e MINIO_ACCESS_KEY=… -e MINIO_SECRET_KEY=… \
  -e MINIO_BUCKET=outcome -e MINIO_ENC_KEY="<ключ из вашего .env>" \
  -e DRY_RUN=1 skayloo/outcome-decrypt-bucket:latest      # затем ещё раз без DRY_RUN
```

Он переписывает зашифрованные объекты открытым текстом на месте, пропускает уже открытые,
ничего не удаляет и безопасен при повторном запуске. Когда напишет `0 rewritten` — убирайте
`MINIO_ENC_KEY` и обновляйтесь.
...
Сохраните и `.env`. Если в нём остался `MINIO_ENC_KEY` от старой установки — держите его,
пока не выполните перевод бакета выше: файлы, загруженные тогда, без него не прочитать.
