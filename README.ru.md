...
Это **открытая self-hosted редакция**. Всё шифруется *в канале связи* (HTTPS/WSS через
Caddy, DTLS-SRTP для медиа), загруженные файлы шифруются *на диске* в MinIO (AES-256-GCM,
ключ ваш). Текст сообщений лежит в вашем Postgres в открытом виде — сервер ваш и база
ваша, но знать об этом стоит до того, как вы пообещаете пользователям обратное.
